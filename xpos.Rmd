...
retire <- all_data %>% mutate(ret = if_else(positionText == "R", 1, 0 )) %>%
                            group_by(year) %>%
...
retire_pos <- all_data %>% filter(grid > 0) %>%
                              filter(gridtot == 20) %>%
...
retiretot <- all_data %>% mutate(ret = if_else(positionText == "R", 1, 0 )) %>%
                            group_by(raceId) %>%
...
posg <- all_data %>% left_join(retiretot, by = "raceId") %>%
